function ApplicationsController($scope, $http) {
  ...
  $scope.load = function () {
    $http.get('/_internal/o-auth-client-application').success(function (data) {
      console.log(data);
      $scope.apps = data;
      $scope.orderProp = 'id';
    });
  ...
  $scope.create = function () {
    $http.post('/_internal/o-auth-client-application',
      {
        name: $scope.name,
  ...
  $scope.delete = function (index, id) {
    $http.delete('/_internal/o-auth-client-application/' + encodeURIComponent(id)).success(function (data, status, headers) {
